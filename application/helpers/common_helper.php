...
{
    $url = "http://mysmsshop.in/V2/http-api.php?apikey=gUx5CwHIYlQF8fhP&senderid=WSALEK&number=$contact_no&message=" . urlencode($message_content) . "&format=json";
    $res = curl_init();
...
// function Placeorder($contact_no, $message_content)
// {
//     $url = "http://mysmsshop.in/V2/http-api.php?apikey=gUx5CwHIYlQF8fhP&senderid=WSALEK&number=$contact_no&message=" . urlencode($message_content) . "&format=json";
//     $res = curl_init();
//     curl_setopt($res, CURLOPT_URL, $url);
...
function mailmsg($to, $subject, $message)
{
	...
	$headers .= 'From:  info@thedaffodilsfarm.com' . "\r\n";
	$headers .= 'Cc: '. $to . "\r\n";
